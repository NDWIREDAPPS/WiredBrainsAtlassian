            ...
            <div className="max-w-4xl mx-auto">
                ...
                <ul className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {whyChooseUs.map((reason, index) => (
                        <li key={index} className="flex items-start gap-4 bg-white p-6 rounded-2xl border border-purple-100 shadow-xs hover:border-purple-300 hover:shadow-md transition-all">
                            <div className="p-1 rounded-full mt-1 shrink-0">
                                <CheckCircle className="h-5 w-5 text-purple-600" />
                            </div>
                            <span className="text-slate-800 font-medium text-lg leading-snug">{reason}</span>
                        </li>
                    ))}
                </ul>
